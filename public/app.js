...
function openRecommendedDemoStep(action) {
  state.drawerOpen = false;
  state.helpPanel = "";
  if (action === "record") {
    state.page = "workbench";
    state.selectedRecordId = "REC-001";
    state.drawerOpen = true;
    state.filters = { ...defaultFilters };
    ...
    state.page = "workbench";
    state.filters = { ...defaultFilters, autoDecision: "知识卡片待审核" };
  } else if (action === "rag") {
    state.page = "rag";
    ragState().query = "压载泵振动升温怎么排查？";
  } else if (action === "agent") {
    state.page = "agent";
    state.agentQuestion = "压载泵振动升温怎么排查？";
  } else if (action === "analysis") {
    state.page = "analysis";
  }
  addLog(`打开推荐演示路径：${action}`);
  saveState();
  render();
}
...
async function loadBuiltInKnowledgePack() {
  const rag = ragState();
  const now = new Date().toLocaleString("zh-CN", { hour12: false });
  ...
  const builtInDocs = rag.documents.filter((doc) => doc.builtIn);
  for (const doc of builtInDocs) {
    await buildRagChunksForDocumentNoRender(doc);
  }
  updateChunkStrategyReport();
  rag.notice = `内置 RAG 文档包已完成：文档加载 ${builtInDocs.length} 份 → 文本解析 → Chunk切分 → Metadata生成 → Embedding → 写入${rag.config.vectorStoreMode === "Aliyun DashVector" ? "DashVector" : "LocalStorage"}。`;
  saveState();
  render();
}
...
async function compareChunkStrategies() {
  updateChunkStrategyReport();
  ragState().notice = "已完成 Chunk 策略对比：段落、句子、Recursive 默认切分。";
  saveState();
  render();
}

function updateChunkStrategyReport() {
  const rag = ragState();
  ...
  return `
    <section class="agent-runtime-page">
      <div class="enterprise-hero panel">
        <div>
          <p class="eyebrow">LangGraph StateGraph Runtime</p>
          <h2>基于 LangGraph StateGraph 编排的船舶设备故障处理 Agent</h2>
          <p class="dashboard-desc">LangGraph 负责流程编排、状态流转和条件分支；Agent 负责船舶设备故障问题处理、工具调用、RAG引用、人工复核和反馈回流。</p>
        </div>
        ...
        <div class="agent-runtime-main">
          <article class="panel enterprise-card">
            ...
            <details class="analysis-raw-detail">
              <summary>查看 State 对象快照</summary>
              <pre>${escapeHtml(JSON.stringify({
                run_id: run.agent_run_id,
                agent_route: run.agent_route,
                route: run.route,
                current_node: run.current_node,
                selected_equipment: run.selected_equipment,
                missing_fields: run.missing_fields,
                risk_level: run.risk_level,
                waiting_human_input: run.waiting_human_input,
...
function renderAgentRuntimeOverview(run, toolCalls, citations) {
  const humanRequired = run.route === "human_review" || run.waiting_human_input || (run.missing_fields || []).length > 0;
  const overview = [
    ["Router", run.agent_route ? `路由到 ${agentRouteName(run.agent_route)}` : "等待运行", run.route_reason || "按关键词和业务意图分流"],
    ["State", run.agent_run_id ? `Run ${run.agent_run_id}` : "未生成", run.status || "等待创建状态"],
    ["Node Trace", run.langgraph?.nodes?.length ? `${run.langgraph.nodes.length} 个节点` : "等待执行", run.current_node || "未进入节点"],
    ["Tool Calls", `${toolCalls.length} 次调用`, toolCalls.length ? toolCalls.map((call) => call.tool || call.tool_name).filter(Boolean).join(" / ") : "等待工具调用"],
    ["RAG Hits", `${citations.length} 条命中`, citations.length ? citations.map((item) => item.document_name || item.title).slice(0, 2).join(" / ") : "等待检索或暂无命中"],
    ["Human Review", humanRequired ? "需要人工复核" : run.agent_run_id ? "暂不需要" : "等待判断", humanRequired ? `原因：${(run.missing_fields || []).join("、") || "低置信度/规则要求"}` : "关键字段和置信度满足当前流程"],
    ["Resume", run.human_review_result ? "已补充并恢复" : humanRequired ? "等待人工补充" : "无需恢复", run.human_review_result ? "使用同一 run_id 继续执行" : "human_review 分支可提交补充字段"],
    ["Final Answer", run.answer ? "已生成" : "待生成", run.answer ? "包含业务建议和引用依据" : "运行后输出排查步骤、SOP和低置信提示"],
    ["Feedback", run.feedback?.type || run.feedback?.status || "待反馈", "反馈会形成知识缺口或待审核知识卡片"]
  ];
  return `
    <article class="panel enterprise-card">
      <div class="section-heading compact">
        <div><p class="eyebrow">LangGraph / Agent Runtime Chain</p><h2>运行链路总览</h2></div>
        <span class="status-pill">${run.agent_run_id ? "已生成Run" : "待运行"}</span>
      </div>
      <div class="agent-runtime-overview">
        ${overview.map(([title, status, desc]) => `
          <article>
...
function renderRagPipelineStatus(rag) {
  const builtInDocs = rag.documents.filter((doc) => doc.builtIn);
  const builtInChunks = rag.chunks.filter((chunk) => builtInDocs.some((doc) => doc.id === chunk.document_id));
  const embeddingReady = builtInChunks.filter((chunk) => chunk.embeddingStatus && !chunk.embeddingStatus.includes("待")).length;
  const vectorReady = rag.vectorStore.status?.includes("写入") || rag.vectorStore.status?.includes("检索完成");
  const steps = [
    ["文档加载", builtInDocs.length ? `${builtInDocs.length} 份已加载` : "未加载", builtInDocs.length > 0],
    ["文本解析", builtInDocs.every((doc) => doc.parseStatus === "解析成功") && builtInDocs.length ? "解析成功" : "等待解析", builtInDocs.length && builtInDocs.every((doc) => doc.parseStatus === "解析成功")],
    ["Chunk切分", builtInChunks.length ? `${builtInChunks.length} 个Chunk` : "未生成", builtInChunks.length > 0],
    ["Embedding生成", embeddingReady ? `${embeddingReady} 个已生成` : "等待生成", embeddingReady > 0],
    ["写入向量库", vectorReady ? rag.vectorStore.status : "未写入", Boolean(vectorReady)],
    ["RAG检索", rag.results.length ? `TopK ${rag.results.length} 条命中` : "等待检索", rag.results.length > 0]
  ];
  return `
    <div class="rag-pipeline-status">
      ${steps.map(([label, status, done], index) => `
  ...
  return `
    <section class="dashboard-page">
      <div class="enterprise-hero panel">
        ...
        <button class="secondary-btn" data-help="center" type="button">帮助中心</button>
      </div>
      ${renderRecommendedDemoPath()}
      <article class="panel enterprise-card">
        <div class="section-heading compact">
...
function renderRecommendedDemoPath() {
  const steps = [
    ["1", "查看压载泵振动温升原始数据", "从一条典型维修脏数据开始，看来源、摘要和当前状态。", "record"],
    ["2", "进入 AI治理工作台", "查看字段抽取、术语标准化、主数据匹配和质量规则校验。", "workbench"],
    ["3", "查看复核与知识卡片", "查看人工复核、DWD标准明细、知识卡片生成和入库状态。", "knowledge"],
    ["4", "进入 RAG 页面", "查看 Chunk、Metadata、Embedding、LocalStorage/DashVector 检索。", "rag"],
    ["5", "运行设备故障 Agent", "输入“压载泵振动升温怎么排查？”，启动设备故障 Agent。", "agent"],
    ["6", "查看 Agent 链路", "看 Router、Tool、RAG命中、human_review、resume、反馈回流。", "agent"],
    ["7", "查看运行分析中心", "查看本次运行评分、成功项、失败项、问题归因和优化建议。", "analysis"]
  ];
  return `
    <article class="panel enterprise-card">
      <div class="section-heading compact">
        <div><p class="eyebrow">Recommended Demo Path</p><h2>推荐演示路径</h2></div>
        <span class="status-pill">面试展示顺序</span>
      </div>
      <div class="demo-path-grid">
        ${steps.map(([num, title, desc, action]) => `
          <button data-demo-path="${action}" type="button">
            <span>${num}</span>
            <strong>${title}</strong>
            <p>${desc}</p>
          </button>
...
function renderWorkbench() {
  ...
  const filteredSummary = workbenchFilteredSummary(tasks);
  return `
    <article class="panel enterprise-card">
      <div class="section-heading compact">
      ...
      ${renderFilters()}
      ${renderWorkbenchResultSummary(filteredSummary)}
...
function workbenchFilteredSummary(tasks) {
  const total = allRecords().length;
  const byDecision = {
    自动通过: tasks.filter((task) => task.ai_decision === "auto_pass").length,
    待人工复核: tasks.filter((task) => task.ai_decision === "human_review").length,
    "待补充/异常": tasks.filter((task) => task.ai_decision === "need_supplement").length,
    知识卡片待审核: tasks.filter((task) => task.ai_decision === "knowledge_review").length,
    已入RAG向量库: tasks.filter((task) => task.ai_decision === "vectorized").length,
    不入库: tasks.filter((task) => task.ai_decision === "rejected").length
  };
  const byQueue = ["设备管理员复核", "维修工程师复核", "质量工程师复核", "工艺工程师复核", "知识管理员审核", "数据管理员复核", "AI自动通过"]
    .map((queue) => [queue, tasks.filter((task) => task.review_queue === queue).length])
    .filter(([, count]) => count > 0);
  return {
    count: tasks.length,
    total,
    ratio: percentValue(tasks.length, total),
    byDecision,
    byQueue
  };
}

function renderWorkbenchResultSummary(summary) {
  return `
    <div class="workbench-result-summary">
      <div>
        <span>当前结果</span>
        <strong>${summary.count} / ${summary.total} 条</strong>
        <em>占总任务 ${summary.ratio}</em>
      </div>
      <div>
        <span>AI决策分布</span>
        <p>${Object.entries(summary.byDecision).map(([label, count]) => `${label} ${count}`).join("｜")}</p>
      </div>
      <div>
        <span>复核队列分布</span>
        <p>${summary.byQueue.map(([label, count]) => `${label} ${count}`).join("｜") || "当前筛选无复核队列"}</p>
      </div>
    </div>
  `;
}
...
function renderHelpPanel(type) {
  const titleMap = { flow: "系统流程图", rules: "流转规则", guide: "使用说明", capabilities: "Demo能力说明", center: "帮助中心" };
  return `
    ...
    <aside class="help-panel">
      ...
      <div class="drawer-body">
        ${type === "flow" ? renderHelpFlow() : type === "rules" ? renderHelpRules() : type === "capabilities" ? renderDemoCapabilities() : type === "center" ? renderUnifiedHelpCenter() : renderHelpGuide()}
      </div>
    </aside>
  `;
}

function renderUnifiedHelpCenter() {
  const blocks = [
    {
      title: "数据治理机制",
      eyebrow: "Governance",
      items: [
        ["数据分层", "企业数据源 → 数据接入层 → ODS原始数据层 → AI治理任务池 → DWD标准明细层 → 知识卡片库 → RAG向量库 → Agent应用层。"],
        ["AI治理职责", "AI负责结构识别、字段抽取、术语标准化、主数据候选匹配、质量规则校验和自动分流。"],
        ["人工复核定位", "人工只处理低置信度、高风险、主数据不唯一、关键字段缺失或企业规则要求确认的数据。"]
      ]
    },
    {
      title: "AI自动通过与分流规则",
      eyebrow: "AI Pass Rule",
      items: [
        ["自动通过前提", "数据可解析、关键字段完整、主数据唯一匹配、标准词表命中、质量规则通过、置信度≥85%、入库规则通过。"],
        ["阈值规则", "≥85% 自动通过进入DWD；60%-85% 进入人工复核；<60% 待补充或驳回。"],
        ["待办分流", "缺设备编码/主数据不唯一给设备管理员；缺维修闭环字段给维修工程师；RT/UT/焊缝给质量工程师；管路/图纸/工艺给工艺工程师；知识发布给知识管理员。"]
      ]
    },
    {
      title: "标准与质量评估",
      eyebrow: "Quality",
      items: [
        ["字段标准", "来源系统、来源单号、原始描述、设备名称、设备编码、故障现象、处理措施、处理结果等字段按标准表校验。"],
        ["质量规则", "缺设备编码、主数据无法唯一确认、缺处理措施、缺处理结果、检测值缺失、描述模糊、来源不可追溯等会触发复核或阻断。"],
        ["置信度计算", "字段完整度、设备匹配度、术语命中度、处理链路完整度、人工复核状态共同影响最终置信度。"]
      ]
    },
    {
      title: "RAG / Chunk / Embedding",
      eyebrow: "RAG",
      items: [
        ["文档型RAG", "PDF/Word/TXT/MD 解析为文本，切 Chunk，补 metadata，生成 embedding，写入 LocalStorage 或 DashVector。"],
        ["Excel治理型RAG", "Excel 不直接入向量库，先做 Sheet 识别、字段映射、清洗标准化和人工复核，再由知识卡片进入 RAG。"],
        ["Chunk评估", "对比按段落、按句子、RecursiveCharacterTextSplitter 的 Chunk 数量、平均长度、TopK命中、相似度和引用准确性。"]
      ]
    },
    {
      title: "LangGraph Agent机制",
      eyebrow: "Agent",
      items: [
        ["Router", "根据问题路由到设备故障、质量异常、数据治理、知识运营、RAG评测或 fallback。"],
        ["Tool Registry", "统一记录 master_data_lookup、rag_retrieve、quality_rule_check、create_review_task、knowledge_card_upsert、feedback_log 等工具的输入、输出、状态和耗时。"],
        ["Human Review / Resume", "Agent 遇到缺设备编码、缺振动值、缺温度值、低置信度等情况进入 human_review，人工补充后使用同一 run_id 恢复执行。"]
      ]
    },
    {
      title: "审核、反馈与运行评估",
      eyebrow: "Audit",
      items: [
        ["审批权限", "一线员工提交问题和反馈；维修工程师补维修字段；设备管理员确认设备编码；知识管理员审核知识卡片；审计管理员查看日志；管理员全权限。"],
        ["知识入库", "知识卡片审核通过后生成 Chunk 和 embedding，写入 DashVector；失败时保留 vector_status 和错误原因。"],
        ["运行评估", "运行分析中心评估 Router、State、Tool、RAG、human_review、resume、引用依据、知识产物、反馈回流和审计日志。"]
      ]
    }
  ];
  return `
...
document.addEventListener("click", (event) => {
  ...
  const demoPathButton = event.target.closest("[data-demo-path]");
  if (demoPathButton) return openRecommendedDemoStep(demoPathButton.dataset.demoPath);
